...
namespace htmledit
{
    ...
    public partial class frmHtmlReportV2 : Form
    {
        ...
        private IDictionary<string, string> _macroListDictionary;
        

        KeysConverter kc;

        public frmHtmlReportV2(VMReportObj rpObj)
        {
            InitializeComponent();
            ...
            _macroListDictionary = rpObj.macroDictionary;
        }
        ...
        private  void MainForm_Load(object sender, EventArgs e)
        {
           ...
           // htmlEditor1.Content.InsertHTMLAtCursor("");
        ...
        {
             // htmlEditorWindow.InsertHTMLWithinTag(reportContent, "Prescription");

            htmlEditor1.Content.UpdateInnerHtmlOfAnElement("Prescription", reportContent);
        ...
        private async void SetMasterTemplate()
        {
            ...
            if (_reportObj != null)
            {
                ...
                {
                    htmlEditor1.Caret.SetCaretPosition(100, 200, true);
                }
        ...
        private void cmbSelectTemplate_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (isLoaded)
            {
                ...
                htmlEditor1.Content.InsertHtml(_rtemplate.TemplateContent,false);
        ...
        private async void btnReportComplete_Click(object sender, EventArgs e)
        {
            ...
            string prescription = htmlEditor1.Content.GetInnerHtmlOfAnElement("Prescription");

            if (string.IsNullOrEmpty(prescription.Trim()))
            {
                MessageBox.Show("Report body is blank.", "RIS", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private async void Convert(string extension)
        {
            ...
            htmlText = htmlEditor1.Content.GetInnerHtmlOfAnElement("Prescription");
        ...
        {
            htmlEditor1.Editor.ShowPageSetupDialog();
        }

        private void BtnPreview_Click(object sender, EventArgs e)
        {
            htmlEditor1.Editor.ShowPrintPreviewDialog();
        }

        private void htmlEditor1_KeyUp(object sender, SpiceLogic.HtmlEditorControl.Domain.BOs.EditorEventArgs.EditorKeyEventArgs e)
        {
            WordCount();
                ...
                if (_macroListDictionary != null)
                {
                    ...
                    switch (e.KeyCode)
                    {
                        case 32:
                            ...
                            break;
                        case 191: // 191 =`/` key
                                  //check if last word found in `_macroListDictionary` dictionary keys
                                  //and replace with dictionary value
                            if (_macroListDictionary.ContainsKey(LastWordTyped))
                            {
                                ReplaceLastWord(_macroListDictionary[LastWordTyped]);
                            }
                            LastWordTyped = "";
                            break;

                        default:
                            LastWordTyped += kc.ConvertToString(e.KeyCode);
                            break;
                    }
        ...
        public void ReplaceLastWord(string NewWord)
        {
            ...
            mshtmlIHTMLDocument2 htmlDocument = htmlEditor1.GetMshtmlDoc2(); //.do as mshtmlIHTMLDocument2;
            mshtmlTextRange textRange = (mshtmlTextRange)htmlDocument.selection.createRange();

            bkmk = textRange.getBookmark();

            textRange.moveStart("word", -2);
            textRange.select();
            textRange.pasteHTML(NewWord);
            textRange.moveToBookmark(bkmk);
        }

        public void WordCount()
        {
            //Debug.Assert(htmlEditor1.Editor. != null, "wb.Document != null");
            //Debug.Assert(wb.Document.Body != null, "wb.Document.Body != null");

            int count = 0;

            if (!string.IsNullOrEmpty(htmlEditor1.Content.GetBodyText()))
            {
                var sec = Regex.Split(htmlEditor1.Content.GetBodyText(), @"\s");
